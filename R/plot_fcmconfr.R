...
get_plot_data <- function(fcmconfr_object) {
  ...
  if (fcmconfr_object$fcm_class == "conventional") {
    ...
    max_y <- max(max(mc_inferences_longer$value), max(aggregate_inferences_longer$value))
    max_y <- (ceiling(max_y*1000))/1000
    min_y <- min(min(mc_inferences_longer$value), min(aggregate_inferences_longer$value))
