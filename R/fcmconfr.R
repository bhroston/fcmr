...
plot.fcmconfr <- function(x, ...) {
  ...
  ggplot() +
    geom_boxplot(data = results, aes(x = value, y = name, fill = analysis_source, color = analysis_source)) +
    geom_point(data = aggregate_inferences_longer, aes(x = value, y = name, shape = analysis_source), position = position_nudge(y = -0.18), size = 2) +
