...
test_that("trying_simulate_tfn", {
  ...
  tfn_adj_matrix <- get_triangular_adj_matrix_from_lower_mode_and_upper_adj_matrices(lower_adj_matrix, mode_adj_matrix, upper_adj_matrix)
  ...
  expect_warning(
    test <- simulate_tfn_with_clamping_or_pulse_only(tfn_adj_matrix = tfn_adj_matrix,
  ...
  expect_no_error(
    test <- infer_ftcm_with_clamping(tfn_adj_matrix = tfn_adj_matrix,
                                     initial_state_vector = initial_state_vector,
